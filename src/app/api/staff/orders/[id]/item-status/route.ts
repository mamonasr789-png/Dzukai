...
import { updateItemStatus } from "../../../../../../lib/server/orderService";
...
const BodySchema = z
  .object({
    ...
    status: z.enum(["NEW", "PREPARING", "READY", "DELIVERING", "COMPLETED", "CANCELLED"]),
  })
  .strict();
...
export async function POST(
  ...
  try {
    const order = await updateItemStatus(id, parsed.data.productId, parsed.data.status, staff);
